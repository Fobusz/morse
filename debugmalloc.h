...
/* initialize a memory block allocated for the user. the start_translation and the end
...
static void *debugmalloc_realloc_full(void *oldmem, size_t newsize, char const *func, char const *expr, char const *file, unsigned line) {
    ...
    /* imitate standard realloc: equivalent to malloc if first paramCheck is NULL */
    if (oldmem == NULL)
